...
public final class PortfolioManager {
    ...
    /**
     * Sync portfolio with Kraken positions.
     */
    public void syncWithKraken(com.trading.broker.KrakenClient client, double defaultTpPercent, double defaultSlPercent) {
        try {
            logger.info("Syncing portfolio with Kraken positions...");
            var positionsFuture = client.getOpenPositionsAsync();
            var json = positionsFuture.get(5, java.util.concurrent.TimeUnit.SECONDS);
            ...
            if (root.has("result")) {
                ...
                while (fields.hasNext()) {
                    var id = fields.next();
                    var pos = result.get(id);
                    
                    String symbol = pos.has("pair") ? pos.get("pair").asText() : id;
                    double qty = pos.has("vol") ? pos.get("vol").asDouble() : 0.0;
                    double cost = pos.has("cost") ? pos.get("cost").asDouble() : 0.0;
                    double entryPrice = qty > 0 ? cost / qty : 0.0;
                    ...
                    logger.info("Synced Kraken position: {} - {} @ ${} (TP: ${}, SL: ${})", 
                        symbol, qty, String.format("%.2f", entryPrice),
                        String.format("%.2f", estimatedTP), String.format("%.2f", estimatedSL));
                }
            }
            
            logger.info("Kraken sync complete: {} positions loaded", syncedCount);
            
        } catch (Exception e) {
            logger.error("Failed to sync portfolio with Kraken", e);
        }
    }
