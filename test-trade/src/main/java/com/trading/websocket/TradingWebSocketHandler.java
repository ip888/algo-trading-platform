...
public final class TradingWebSocketHandler {
    ...
    public static void onConnect(WsConnectContext ctx) {
        ...
        logger.info("WebSocket connected: {} (total: {})", ctx.getSessionId(), sessions.size());
        
        // Don't send immediately - let connection fully establish first
        // Client will request data via REST API or ping message
    }
    ...
    public static void onClose(WsCloseContext ctx) {
        ...
        logger.info("WebSocket closed: {} (remaining: {})", 
            ctx.getSessionId(), sessions.size());
    }
    
    public static void onError(WsErrorContext ctx) {
        logger.error("WebSocket error for session {}", ctx.getSessionId());
        sessions.remove(ctx.getSessionId());
    }
    
    public static void onMessage(WsMessageContext ctx) {
        String message = ctx.message();
        logger.debug("Received message from {}: {}", ctx.getSessionId(), message);
        ...
        try {
            ...
            switch (type) {
                ...
                case "refresh" -> {
                    ...
                    logger.info("Client {} requested data refresh", ctx.getSessionId());
                }
                case "subscribe" -> {
                    // Client wants to subscribe to specific symbols
                    if (msgNode.has("symbols")) {
                        String symbols = msgNode.get("symbols").asText();
                        logger.info("Client {} subscribed to symbols: {}", ctx.getSessionId(), symbols);
     ...
     */
    public static void broadcastAccountData(double equity, double buyingPower, 
    ...
    private static void sendToSession(WsConnectContext ctx, ObjectNode message) {
        try {
            ...
            logger.error("Failed to serialize and send message to session {}", ctx.getSessionId(), e);
        }
    }
    ...
    private static void sendToSession(WsConnectContext ctx, String message) {
        try {
            ...
            logger.error("Failed to send message to session {}", ctx.getSessionId(), e);
            sessions.remove(ctx.getSessionId());
        }
    }
