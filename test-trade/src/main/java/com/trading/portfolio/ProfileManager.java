...
public class ProfileManager implements Runnable {
    ...
    public ProfileManager(
        ...
        portfolio.syncWithAlpaca(client, profile.takeProfitPercent(), profile.stopLossPercent());
        ...
        this.regimeDetector = new MarketRegimeDetector(client, config, marketAnalyzer);
        ...
        this.correlationCalculator = new com.trading.analysis.CorrelationCalculator(client);
        ...
        this.smartOrderRouter = new com.trading.execution.SmartOrderRouter(config, client);
    ...
    private void runTradingCycle() throws Exception {
        ...
        if (portfolioRiskManager.shouldHaltTrading(accountEquity)) {
            ...
            var risk = portfolioRiskManager.assessRisk(client, accountEquity);
    ...
    private void broadcastAccountData(double profileEquity) {
        try {
            // Fetch TOTAL account equity and buying power from Alpaca (not just profile's allocation)
            double totalEquity = profileEquity; // Default to profile equity
            double buyingPower = 0.0;
            
            try {
                var account = client.getAccount();
                totalEquity = account.get("equity").asDouble(); // Use TOTAL account equity
                buyingPower = account.get("buying_power").asDouble();
            } catch (Exception e) {
                logger.debug("[{}] Failed to fetch account data, using profile equity", profile.name(), e);
            }
            
            // Get profit targets from config (not hardcoded!)
            double mainTakeProfit = config.getMainTakeProfitPercent();
            double expTakeProfit = config.getExperimentalTakeProfitPercent();
            double stopLoss = profile.stopLossPercent(); // From profile config
            
            TradingWebSocketHandler.broadcastAccountData(
                totalEquity,  // Use TOTAL account equity, not profile allocation
                buyingPower,
                mainTakeProfit,
                expTakeProfit,
                stopLoss
    ...
    private void checkAllPositionsForProfitTargets(String profilePrefix) {
        ...
        try {
            ...
            var tempPosition = new TradePosition(
                ...
                entryPrice,
                qty,
                entryPrice * 0.98, // Approximate stop
                entryPrice * 1.01, // Approximate target
    ...
    private void broadcastProfitTargetsData() {
        ...
        try {
            ...
            logger.debug("Broadcasting profit targets for {} positions", positions.size());
            ...
            for (var pos : positions) {
                ...
                logger.info("Position {} using target from config: {}%", pos.symbol(), targetPercent);
            ...
            if (!targets.isEmpty()) {
                logger.info("Broadcasting {} profit targets", targets.size());
