 ...
 * 
 * Modern design:
 * - Uses LockSupport.parkNanos instead of Thread.sleep for non-blocking waits
...
public class AutoRecoveryManager {
    ...
    public AutoRecoveryManager(Config config, AlpacaClient client) {
        this.config = config;
        this.client = client;
        logger.info("🤖 AutoRecoveryManager initialized - Self-healing enabled (modern non-blocking design)");
    }
    
    /**
     * Detect and recover from API connection failures.
     * Uses LockSupport.parkNanos for non-blocking exponential backoff.
     */
    public boolean recoverFromApiFailure(Exception error) {
        ...
        try {
            // Wait with exponential backoff using non-blocking park
