...
use crate::types::{
    OrderSide, Position, PositionWithPnl, PositionsResponse, StatusResponse, Trade,
    TradingCycleResult, TradingStateData,
};
use chrono::Utc;
...
impl TradingEngine {
    ...
    async fn scan_for_entries(
        ...
        // Market regime threshold: allow trades if BTC > -1%
        const BTC_REGIME_THRESHOLD: f64 = -1.0;
        ...
        worker::console_log!(
            "Portfolio: ${:.2} | Tier: {} | Risk: {}% | Max Positions: {}",
            total_portfolio,
            tier_params.tier.name(),
            tier_params.risk_per_trade_percent,
            tier_params.max_positions
        );
        ...
        let max_new = self
            .strategy
            .max_new_positions(total_portfolio, state.positions.len());
        if max_new == 0 {
            worker::console_log!(
                "At max positions ({}/{} for {} tier)",
                state.positions.len(),
                tier_params.max_positions,
                tier_params.tier.name()
            );
        ...
        // Market regime filter: check if BTC is above threshold
        ...
        if self.config.enable_market_regime_filter {
            ...
            if btc_stats.change_24h < BTC_REGIME_THRESHOLD {
                worker::console_log!(
                    "Market regime: BEARISH (BTC {:.2}% < {:.0}% threshold) - skipping new entries",
                    btc_stats.change_24h,
                    BTC_REGIME_THRESHOLD
                );
                return Ok(());
            }
            worker::console_log!(
                "Market regime: OK (BTC {:.2}% >= {:.0}% threshold) - scanning for entries",
                btc_stats.change_24h,
                BTC_REGIME_THRESHOLD
            );
        }
        ...
        for symbol in &self.config.symbols {
            ...
            if self
                .strategy
                .should_enter(&analysis, state.positions.len(), total_portfolio)
            {
                ...
                let sizing = self.strategy.calculate_position_size(
                    total_portfolio,
                    usd_balance,
                    volatility_factor,
                );
                ...
                if !sizing.can_trade {
                    worker::console_log!(
                        "{}: Can't trade - {}",
                        symbol,
                        sizing.reason.unwrap_or_default()
                    );
                    continue;
                }
                ...
                let (stop_loss_price, take_profit_price, sl_pct, tp_pct) = self
                    .strategy
                    .calculate_dynamic_tp_sl(stats.price, range_percent);

                worker::console_log!(
                    "{}: Opening ${:.2} position | SL: ${:.2} (-{:.1}%) | TP: ${:.2} (+{:.1}%)",
                    symbol,
                    position_size,
                    stop_loss_price,
                    sl_pct,
    ...
    pub async fn get_positions_with_pnl(
        &self,
