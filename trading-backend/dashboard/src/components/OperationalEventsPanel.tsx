...
function OperationalEventsPanel() {
  ...
  const setMarketHours = useTradingStore(state => state.setMarketHours);
  const setHealthStatus = useTradingStore(state => state.setHealthStatus);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Fetch market hours
        const hoursRes = await fetch(`${CONFIG.API_BASE_URL}/api/market/hours`);
        if (hoursRes.ok) {
        ...
        // Fetch system health
        const healthRes = await fetch(`${CONFIG.API_BASE_URL}/api/system/health`);
        if (healthRes.ok) {
  ...
  }, [setMarketHours, setHealthStatus]);
