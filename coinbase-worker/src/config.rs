...
impl Config {
    ...
    pub fn from_env(env: &Env) -> Result<Self> {
        Ok(Self {
            ...
            take_profit_percent: env
                .var("TAKE_PROFIT_PERCENT")
                .map(|v| v.to_string().parse().unwrap_or(2.0))
                .unwrap_or(2.0),

            stop_loss_percent: env
                .var("STOP_LOSS_PERCENT")
                .map(|v| v.to_string().parse().unwrap_or(1.5))
                .unwrap_or(1.5),

            trailing_stop_percent: env
                .var("TRAILING_STOP_PERCENT")
                .map(|v| v.to_string().parse().unwrap_or(0.75))
                .unwrap_or(0.75),

            // Volatility-Adaptive TP/SL
            atr_sl_multiplier: env
                .var("ATR_SL_MULTIPLIER")
                .map(|v| v.to_string().parse().unwrap_or(1.0))
                .unwrap_or(1.0),

            atr_tp_multiplier: env
                .var("ATR_TP_MULTIPLIER")
                .map(|v| v.to_string().parse().unwrap_or(2.0))
                .unwrap_or(2.0),
            ...
            daily_trade_limit: env
                .var("DAILY_TRADE_LIMIT")
                .map(|v| v.to_string().parse().unwrap_or(30))
                .unwrap_or(30),

            max_consecutive_errors: env
                .var("MAX_CONSECUTIVE_ERRORS")
                .map(|v| v.to_string().parse().unwrap_or(5))
                .unwrap_or(5),

            enable_trend_filter: env
                .var("ENABLE_TREND_FILTER")
                .map(|v| v.to_string().to_lowercase() == "true")
                .unwrap_or(true),

            enable_volume_filter: env
                .var("ENABLE_VOLUME_FILTER")
                .map(|v| v.to_string().to_lowercase() == "true")
                .unwrap_or(true),

            enable_market_regime_filter: env
                .var("ENABLE_MARKET_REGIME_FILTER")
                .map(|v| v.to_string().to_lowercase() == "true")
            ...
            max_position_age_hours: env
                .var("MAX_POSITION_AGE_HOURS")
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        if self.stop_loss_percent <= 0.0 {
            return Err(TradingError::Config(
                "stop_loss_percent must be positive".into(),
            ));
        }
        if self.max_risk_per_trade_percent <= 0.0 || self.max_risk_per_trade_percent > 10.0 {
            return Err(TradingError::Config(
                "max_risk_per_trade_percent must be 0-10%".into(),
            ));
        }
